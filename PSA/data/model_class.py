...
class RNN(nn.Module):
    def __init__(self,hidden_units, n_class=1, n_hidden=1):
        ...
        self.rnn = nn.RNN(input_size=n_class, hidden_size=n_hidden,dropout=0.1, batch_first= True, num_layers=1)
    ...
    def forward(self, X):
        ...
        final_output = torch.cat((outputs, V), dim=1)
...
class RNN_full(nn.Module):
    ...
    def forward(self, X):
        ...
        outputs, hidden = self.rnn(Xseq)
        outputs = outputs[:, :, 0]  # 최종 예측 Hidden Layer
        final_output = outputs#torch.cat((outputs), dim=1)
        result = self.mlp(final_output)  # 최종 예측 최종 출력 층
        return result
